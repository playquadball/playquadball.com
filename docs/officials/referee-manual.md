...
```mermaid
graph TD
    A[Keeper with green headband enters own keeper zone.] --> B{"Does keeper have sole possession of the volleyball?"};
    B -- No --> C[Keeper can be legally tackled. Keeper IS immune to dodgeballs.];
    B -- Yes --> D{"Has the offensive team already possessed the volleyball outside the zone on this drive?"};
    D -- Yes --> E[Keeper has NO immunity. Can be tackled and beat.];
    D -- No --> F[Keeper is PROTECTED. Cannot be tackled/stolen from. Is IMMUNE to dodgeballs.];
    F --> G{"Does the keeper or a teammate then possess the ball outside the zone?"};
    G -- Yes --> E;
    G -- No --> F;
```
